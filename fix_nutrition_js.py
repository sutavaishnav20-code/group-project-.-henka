...
# 1. Replace AI Food Scanner Logic completely and add new functions

ai_food_js_pattern = r"        const dropZone = document\.getElementById\('dropZone'\);.*?}\);(?=\s*// --- Mentors Logic ---)"

ai_food_js_new = """        // --- Nutrition Tabs Logic ---
        window.switchNutritionTab = function(tabId) {
        ...
        function updateWaterDisplay() {
            waterDisplay.innerText = dailyWater.toFixed(2) + ' L';
        }
        updateWaterDisplay();
...
content = re.sub(ai_food_js_pattern, ai_food_js_new, content, flags=re.DOTALL)
